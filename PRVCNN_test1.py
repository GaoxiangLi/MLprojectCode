    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(total_batch - 1):
                ...
                _, y_pred, y_true, losses = sess.run([train_op, y, y_, loss], feed_dict={x: xs, y_: ys, drop_rate: 1})
        ...
        for i in range(TESTING_EXAMPLE):
            ...
            _, y_pred_result = sess.run([train_op, y_softmax],
                                        feed_dict={x: x_test, drop_rate: 1})
...
def test1(args):
    ...
    with tf.Session() as sess:
        ...
        print("training model prediction score for test1 saved in ./result path")
        y_true = np.argmax(test_label, axis=1)
        y_pred = np.argmax(y_pred, axis=1)
        acc = accuracy_score(y_true, y_pred)
        precision_2 = precision_score(y_true, y_pred, average='weighted')
